...
class AuditLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def export_csv(self, request):
        try:
            ...
            writer.writerow(['Timestamp', 'Actor', 'Action', 'Entity Type', 'Entity ID', 'Details', 'IP Address'])
            ...
            logger.info("export_csv: exported %d rows for org %s", qs.count(), request.user.organization.id)
            return response

        except Exception:
            logger.exception("export_csv: failed for user %s", request.user.id)
